class Index extends App.ControllerIntegrationBase
  featureIntegration: 'twilio_integration'
  featureName: 'Twilio'
  featureConfig: 'twilio_config'
  description: [
...
class Form extends App.Controller
  ...
  currentConfig: ->
    config = App.Setting.get('twilio_config')
    if !config.outbound
      config.outbound = {}
  ...
  setConfig: (value) ->
    App.Setting.set('twilio_config', value, {notify: true})

  render: =>
    @config = @currentConfig()

    @html App.view('integration/twilio')(
      config: @config
      twilio_token: App.Setting.get('twilio_token')
    )
...
class State
  @current: ->
    App.Setting.get('twilio_integration')

App.Config.set(
  'IntegrationTwilio'
  {
    name: 'Twilio'
    target: '#system/integration/twilio'
    description: 'Building blocks to add messaging, voice, and video in your web and mobile applications.'
    controller: Index
    state: State
  }
  'NavBarIntegrations'
